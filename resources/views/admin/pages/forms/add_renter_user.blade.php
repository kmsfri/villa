...
@section('content_add_form')
    ...
    <div class="form-group{{ $errors->has('avatar_dir') ? ' has-error' : '' }}">
        ...
        <div class="col-md-4 pull-right">
            <img id="img_preview" class="{{ isset($u->avatar_dir) ? '' : 'hide' }}" src="{{ isset($u->avatar_dir) ? url('/images/users/user-uploads/user-pics/'.$u->avatar_dir) : '#' }}" alt="تصویر پروفایل" autocomplete="off" />
        </div>
    </div>

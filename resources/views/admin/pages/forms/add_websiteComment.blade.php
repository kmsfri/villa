    ...
    <div class="form-group{{ $errors->has('comment_status') ? ' has-error' : '' }}">
        ...
        <div class="col-md-6 pull-right">
            <select  name='comment_status' class='selectpicker form-control pull-right' autocomplete="off">
                <option @if(old('comment_status' , isset($comment->comment_status) ? $comment->comment_status : '')==0) selected @endif value="0" >جدید</option>
                <option @if(old('comment_status' ,isset($comment->comment_status) ? $comment->comment_status : '' )==1) selected @endif value="1" >بررسی شده</option>
                <option @if(old('comment_status' ,isset($comment->comment_status) ? $comment->comment_status : '' )==2) selected @endif value="2" >پذیرفته شده</option>
                <option @if(old('comment_status' ,isset($comment->comment_status) ? $comment->comment_status : '' )==3) selected @endif value="3" >رد شده</option>
            </select>
